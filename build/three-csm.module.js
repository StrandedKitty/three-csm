import { Matrix4, Vector3, ShaderChunk, Box3Helper, Box3, Color, MeshBasicMaterial, DoubleSide, Mesh, PlaneBufferGeometry, Group, BufferGeometry, BufferAttribute, LineSegments, LineBasicMaterial, Object3D, DirectionalLight, Vector2, MathUtils } from 'three';
...
var CSMHelper = /** @class */ (function (_super) {
    ...
    CSMHelper.prototype.update = function () {
        ...
        while (cascadeLines.length < cascades) {
            var cascadeLine = new Box3Helper(new Box3(), new Color(0xffffff));
            var planeMat = new MeshBasicMaterial({ transparent: true, opacity: 0.1, depthWrite: false, side: DoubleSide });
            var cascadePlane = new Mesh(new PlaneBufferGeometry(), planeMat);
            var shadowLineGroup = new Group();
            var shadowLine = new Box3Helper(new Box3(), new Color(0xffff00));
...
var CSM = /** @class */ (function () {
    function CSM(camera, parent, args) {
        ...
        this.lightDirection = (_f = args.lightDirection) !== null && _f !== void 0 ? _f : new Vector3(1, -1, 1).normalize();
